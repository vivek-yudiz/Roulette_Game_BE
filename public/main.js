...
let ctx;
let startAngle = 0;
const numbers = [0, 32, 15, 19, 4, 21, 2, 25, 17, 34, 6, 27, 13, 36, 
...
function joinGame() {
    return api.post('/api/v1/roulette/join')
        .then(response => {
...
function createNumbersGrid() {
    ...
    for (let i = 1; i <= 36; i++) {
        ...
        div.className = `number ${isRed(i) ? 'red' : 'black'}`;
        div.textContent = i;
...
function clearBets() {
    ...
    betIndicators.forEach(indicator => indicator.remove());
    
    // Reset spin button state - IMPORTANT CHANGES HERE
    try {
        const spinBtn = document.getElementById('spin');
        if (spinBtn) {
            spinBtn.disabled = true;
            console.log('Spin button disabled successfully');
        }
    } catch (error) {
        console.warn('Error updating spin button:', error);
    }
}
...
class RouletteWheel {
    ...
    drawWheel() {
        ...
        for(let i = 0; i < this.wheelNumbers.length; i++) {
            ...
            this.ctx.fillStyle = this.getNumberColor(number) === 'green' ? '#27ae60' : 
                                this.getNumberColor(number) === 'red' ? '#e74c3c' : '#2c3e50';
...
function showResultPopup(result) {
    ...
    const { number, color, winners } = result;
    const currentUserId = '6746a3a6d58ec66ee0c99895'; // Your current user ID
    const userWin = winners.some(winner => winner.userId === currentUserId);
    ...
    // Set popup content with animation classes
    popup.innerHTML = `
        <div class="popup-content animate">
            <h2 class="animate-text">${userWin ? '🎉 You Won! 🎉' : '😔 You Lost!'}</h2>
            <div class="result-number ${color} animate-spin">${number}</div>
            ${userWin 
                ? `<div class="win-amount animate-bounce">+${winners.find(w => w.userId === currentUserId).winAmount} chips</div>`
                : '<div class="loss-message animate-fade">Better luck next time!</div>'
            }
